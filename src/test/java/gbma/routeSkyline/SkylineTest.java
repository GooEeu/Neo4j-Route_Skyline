...
class SkylineTest {
	...
	private static final String SETUP_SAMPLE_GRAPH= "CREATE\n" + 
			...
			"(n0)-[:WAY { dist:8, time:1 }]->(n3), " +
			"(n0)-[:WAY { dist:1, time:1 }]->(n4), " +
			"(n0)-[:WAY { dist:7, time:4 }]->(n5), " +
			"(n0)-[:WAY { dist:7, time:4 }]->(n6), " +
	...
	public void testExpander(){
		TestDatabase.testAllPathsCall(db,
				"MATCH (from {name:'n0'}), (to {name:'n6'}) " +
						"CALL gdma.routeSkyline.expander(from, to, 'WAY') yield relationship " +
						"RETURN relationship" ,
				row ->  {
					System.out.println(row.get("relationship").toString());
				}
		);
	}
